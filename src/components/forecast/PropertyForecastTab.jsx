...
/**
 * PropertyForecastTab Component
 * Analyzes property and asset opportunities based on:
 * - Number 1 (Real Estate Initiator)
 * - Number 8 (Even 8 pattern for bulk gains)
 * - Annual and Maha Dasha influences
 * - Categories: Property, Expansion, Cash Flow, Vehicle & Luxury
 */
const PropertyForecastTab = ({ report, dashaReport, targetDate }) => {
    ...
    const propertyAnalysis = useMemo(() => {
        ...
        if (annualDasha.dashaNumber === 1 || mahaDasha.dashaNumber === 1) {
            if (count1 === 1) {
                specialInsights.push({
                    title: "Number 1 – The Real Estate Initiator (Single 1)",
                    text: "Positive for property acquisition. During this Dasha, you are likely to buy property (residential or commercial) or be presented with opportunities in real estate professions. Success is moderate but stable; better to buy rather than sell.",
                    status: 'Green'
                });
            } else if (count1 > 1 && destinyNumber === 1) {
                specialInsights.push({
                    title: "Number 1 – The Real Estate Initiator (Multiple 1s with Destiny 1)",
                    text: "Extremely favorable for real estate business. You are likely to consistently earn profits from property investments and are skilled in handling deals. Real estate can be a core career or wealth-building avenue.",
                    status: 'Green'
                });
            } else if (count1 > 1 && destinyNumber !== 1) {
                ...
                specialInsights.push({
                    title: "Number 1 – The Real Estate Initiator (Missing 1)",
                    text: "The Dasha of Number 1 may still bring opportunities for property purchase or home renovation. However, it's not advisable to pursue real estate as a business venture. Focus on personal use property only.",
                    status: 'Yellow'
                });
            }
        }

        // Rule for Number 8 - Even Pattern Analysis
            ...
            switch (annualDasha.dashaNumber) {
                case 1:
                    if (count1 === 1 || (count1 > 1 && destinyNumber === 1) || count1 === 0) {
                        opportunities.push({
                            type: 'Property',
                            text: 'Highly auspicious for acquiring property. Ideal time for purchases, title finalization, or land ownership initiation. Favorable for entering real estate business.'
                        });
                    }
                    break;
                case 3:
                    opportunities.push({
                        type: 'Expansion',
                        text: 'Promotes buying a second home, relocating to a bigger place, or investing in real estate for long-term returns. Also creates harmony in the family.'
                    });
                    break;
                case 5:
                    opportunities.push({
                        type: 'Cash Flow',
                        text: 'Brings financial liquidity, bonuses, or inheritance. Can be used for property down payments or loan clearance.'
                    });
                    break;
                case 6:
                    opportunities.push({
                        type: 'Vehicle & Luxury',
                        text: 'Activates vehicle upgrades, home renovations, and luxury asset buying to enhance material comforts.'
                    });
                    break;
                case 4:
        ...
        // Luxury Purchase in Dasha 6 (outside Maha 8)
        ...
        if (annualDasha.dashaNumber === 6 && isPositive6(baseKundliGrid[6] || 0, destinyNumber) && !isMaha8Active) {
            opportunities.push({
                type: 'Luxury Purchase',
                text: 'The Annual Dasha of 6 may lead to the purchase of a luxury item, such as a car, branded gadgets, or premium home upgrades.'
            });
        }
    ...
    const InsightBlock = ({ title, text, status }) => {
        const colorClass = status === 'Green' ? 'border-green-500 bg-green-900/20 text-green-300' : status === 'Yellow' ? 'border-yellow-500 bg-yellow-900/20 text-yellow-300' : 'border-red-500 bg-red-900/20 text-red-300';
        return (
        ...
        <div className="space-y-6">
            <Card>
                ...
                <div className="space-y-4">
                    ...
                    {propertyAnalysis.opportunities.length > 0 && (
                        <div className="p-4 bg-green-900/50 rounded-lg">
                            ...
                            <ul className="list-disc list-inside text-green-300 space-y-2">
                                {propertyAnalysis.opportunities.map((opp, i) => (
                                    <li key={i}>
                    ...
                    {propertyAnalysis.opportunities.length === 0 &&
                        propertyAnalysis.warnings.length === 0 &&
                        propertyAnalysis.specialInsights.length === 0 && (
                            <p className="text-center text-yellow-200/80 p-4">
                                No specific asset triggers are active for this period based on the current Dasha. General financial prudence is advised.
                            </p>
                        )}
                </div>
            </Card>
