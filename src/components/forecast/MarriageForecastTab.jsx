...
import DATA from '../../data/compatibilityData';

/**
 * MarriageForecastTab Component
 * Analyzes marriage possibilities based on:
 * - Maha Dasha influences (long-term)
 * - Annual Dasha influences (yearly)
 * - Favorable dashas: 3, 6, 7, 8 (even)
 * - Personality compatibility by Destiny Number
 * - Year-wise marriage probability (age 20-40)
 */
        ...
        <div className="space-y-6">
            <Card>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        {marriageAnalysis.annualDashaInsights.length > 0 ? marriageAnalysis.annualDashaInsights.map((item, i) => (
            ...
            <Card>
                ...
                <p className="mb-4 text-gray-300">
                    As a Destiny Number <span className="font-bold text-yellow-400">{report.destinyNumber}</span>, your core life path energy influences who you connect with most naturally. {compatibility.description}
                </p>
                <div className="grid md:grid-cols-3 gap-4 text-center">
                    <div className="bg-green-500/10 p-3 rounded-lg">
                        <h4 className="font-bold text-green-400 mb-2">Good Compatibility</h4>
                        <p className="text-2xl font-bold text-gray-200">{compatibility.good.join(', ')}</p>
                    </div>
                    <div className="bg-yellow-500/10 p-3 rounded-lg">
                        <h4 className="font-bold text-yellow-400 mb-2">Neutral</h4>
                        <p className="text-2xl font-bold text-gray-200">{compatibility.neutral.join(', ')}</p>
                    </div>
                    <div className="bg-red-500/10 p-3 rounded-lg">
                        <h4 className="font-bold text-red-400 mb-2">{compatibility.not ? 'Not Compatible' : 'Avoid'}</h4>
                        <p className="text-2xl font-bold text-gray-200">{(compatibility.not || compatibility.avoid || []).join(', ')}</p>
                    </div>
                </div>
            </Card>
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto pr-2">
                    {marriageProbabilityChart.map(item => (
                        <div key={item.age} className="flex items-center justify-between p-2 bg-gray-900/50 rounded-md">
                            ...
                            <div className={`px-3 py-1 text-sm font-semibold rounded-full ${item.status === 'Green' ? 'bg-green-500/20 text-green-300' :
