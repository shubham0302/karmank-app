...
import { reduceToSingleDigit } from '../../utils/calculations';

/**
 * NlgChildBirthForecast Component
 * Generates AI-powered natural language forecast using Gemini API
 */
const NlgChildBirthForecast = ({ analysis }) => {
    ...
    useEffect(() => {
        ...
        const generateText = async () => {
            ...
            try {
                // IMPORTANT: Replace with your actual Gemini API call logic
                // This is a mock response for demonstration purposes.
                // In a real application, you would use fetch() to call the Gemini API endpoint.
                let chatHistory = [];
                ...
                // Fallback to a simpler, rule-based text if API fails
        ...
        generateText();
    }, [analysis]); // Rerun when analysis data changes
...
/**
 * ChildBirthForecastTab Component
 * Analyzes childbirth possibilities based on:
 * - Annual Dasha 3 (favorable conditions)
 * - Even 8 pattern (auspicious events)
 * - Female-specific checks (888, 4, 77, 22, 66 patterns to avoid)
 * - Delivery date analysis (avoid 4, 8; prefer 1, 3, 5, 6)
 */
const ChildBirthForecastTab = ({ report, dashaReport, gender, targetDate }) => {
    ...
    const StatusBlock = ({ status, children }) => {
        const colorClass = status === 'Green' ? 'bg-green-500/20 text-green-300' : status === 'Yellow' ? 'bg-yellow-500/20 text-yellow-300' : status === 'Red' ? 'bg-red-500/20 text-red-300' : 'bg-gray-700/50 text-gray-300';
